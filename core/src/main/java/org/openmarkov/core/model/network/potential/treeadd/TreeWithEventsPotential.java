...
import org.openmarkov.core.exception.ThereMustBeJustOneEventInConfiguration;
...
public class TreeWithEventsPotential extends Potential implements DESSimulablePotential {
    ...
    public TreeWithEventsPotential(List<Variable> variables, PotentialRole role) {
        ...
        events = variables.subList(1, variables.size())
                          .stream()
                          .filter(variable -> variable.getVariableType() == VariableType.EVENT)
                          .collect(Collectors.toList());
        noEvents = variables.subList(1, variables.size())
                            .stream()
                            .filter(variable -> variable.getVariableType() != VariableType.EVENT)
                            .collect(Collectors.toList());
        trees = new HashMap<>();
    ...
    public static boolean validate(Node node, List<Variable> variables, PotentialRole role) {
        ...
        boolean validate = variables.subList(1, variables.size())
                                    .stream()
                                    .anyMatch(variable -> variable.getVariableType() == VariableType.EVENT);
        return validate;
    ...
    public double sampleConditionedVariable(double[] randomNumbers, EvidenceCase parents) throws OpenMarkovException {
        List<Variable> eventVariables = parents.getVariables()
                                               .stream()
                                               .filter(variable -> variable.getVariableType() == VariableType.EVENT)
                                               .toList();
        if (eventVariables.size() != 1) {
            throw new ThereMustBeJustOneEventInConfiguration(this, eventVariables.size());
        }
        //
        ...
        return getTree(eventVariables.getFirst()).sampleConditionedVariable(randomNumbers, parents);
    }
