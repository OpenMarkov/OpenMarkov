...
public class Cache {
	...
	public double getScore(PNEdit edit) {
        ...
        // InvertLinkEdit(tail→head) is approximated as Remove(tail→head) + Add(head→tail).
        // This assumes score decomposability: the inversion score equals the sum of
        // the individual remove and add scores. Valid for decomposable metrics (BIC, K2, etc.)
        // but may not hold for non-decomposable metrics like Accuracy.
        return deltaScoreRem[orderedVariables.get(tail)][orderedVariables.get(head)]
