...
Every information that a java class generates and the developer deems as valuable (debugging,
execution time) should be written to that log.

#### Log priority levels

Every log has some levels to mark the importance of the information, in log4j they are (extracted
...
Setting the log to show any priority level (e.g. WARN) will show every more serious level also (e.g.
ERROR and FATAL).
...
Finally, the log is written in the folder that contains OpenMarkov per session, overwriting the
previous log.