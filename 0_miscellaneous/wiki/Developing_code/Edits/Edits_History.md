...
If we were to execute ``redo`` at this point, we would ``redo`` the left-most edit in the
Undone-stack,
...
When **executing**[^1] an edit, the edit is pushed to the right of the done-stack, and the
undone-stack
is cleared.
...
``probNet.getPNESupport().cancelLastSubEditHistory();``, then the done edits (The links ``A`` =>
``B``,
