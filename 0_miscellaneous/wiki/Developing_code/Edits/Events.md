...
- ``afterUndoingEdit(PNEdit edit)``: Happens after undoing edit.
  <br>The undo operation shall not fail, and therefore there is no
  ``beforeUndoingEdit(PNEdit edit)``
  event.
- ``afterRedoingEdit(PNEdit edit)``: Happens after redoing edit.
  <br>The redo operation shall not fail, and therefore there is no
  ``beforeRedoingEdit(PNEdit edit)``
  event.
