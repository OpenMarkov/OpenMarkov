...
A constraint is a condition(s) that a probabilistic network may fulfill or not.
...
* ``boolean checkProbNet(ProbNet probNet)`` - returns whether the given ProbNet fulfills the
  constraint

* ``boolean checkEdit(ProbNet probNet, PNEdit edit)`` - returns whether the ProbNet will still
  fulfill the constraint after applying the action.
...
is defined in an appendix of the
[ProbModelXML format's technical report](http://www.cisiad.uned.es/techreports/ProbModelXML.php).
