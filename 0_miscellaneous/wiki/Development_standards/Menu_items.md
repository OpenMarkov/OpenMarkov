...
If you are creating a menu item you surely want it to execute an action. If you flip the switch,
you expect it to turn on the lights, either with the switch next to the door or the switch above
your bed.
That is important, you can have ten different switches (items) but the command is one:
"turn on, lights!".
...
| Where do you want it? | To add the item, modify                   | To enable/disable, it modify:                                   | The default listener of the menu item is                       |
|-----------------------|-------------------------------------------|-----------------------------------------------------------------|----------------------------------------------------------------|
| **Main menu**         | `MainMenu` class                          | The `updateOptions()` methods of `MainPanelMenuAssistant` class | The `actionPerformed()` method of `MainPanelListenerAssistant` |
| **Toolbar**           | The specific class of the toolbar         | The `updateOptions()` methods of `MainPanelMenuAssistant` class | The `actionPerformed()` method of `MainPanelListenerAssistant` |
| **Contextual menu**   | The specific class of the contextual menu | The specific class of the contextual menu                       | The `actionPerformed()` method of `MainPanelListenerAssistant` |

## **Adding**
> You can easily do every step looking at the code of other menu items in the same menu.

1. Locate the desired menu class, these are the possible ones as for now (June 2018):
