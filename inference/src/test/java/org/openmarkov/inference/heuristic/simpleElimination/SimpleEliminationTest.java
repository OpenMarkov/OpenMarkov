...
/**
 * Tests for the SimpleElimination heuristic (fewest neighbors).
 *
 * @author Manuel Arias
 */
public class SimpleEliminationTest {
	...
	public void testChainEliminatesExtremeFirst() {
		...
		SimpleElimination heuristic = createHeuristic(probNet);

		// In A->B->C, A and C have 1 neighbor, B has 2.
		// The heuristic should pick one of the extremes first.
		Variable first = heuristic.getVariableToDelete();
		assertNotNull(first);
		assertTrue(first.getName().equals("A") || first.getName().equals("C"),
				"Should eliminate the node with fewest neighbors first (an extreme)");
	}
	...
	public void testAsiaNetworkComplete() {
		...
		// VisitToAsia has only 1 neighbor (Tuberculosis), should be eliminated early.
		assertTrue(order.indexOf("VisitToAsia") < 4,
				"VisitToAsia (1 neighbor) should be eliminated in the first half");
	}

	private SimpleElimination createHeuristic(ProbNet probNet) {
		List<List<Variable>> variablesToEliminate = new ArrayList<>();
		variablesToEliminate.add(new ArrayList<>(probNet.getVariables()));
		return new SimpleElimination(probNet, variablesToEliminate);
	}
